...
const FaqDetail = () => {
    ...
    const onModi = () => {
        if (!router.query.type) router.push('/board/faq/1?type=modi');
        else {
    ...
    return (
        <Wrapper>
            <Detail>
                <Descriptions title={<Title>FAQ 글 상세</Title>} bordered column={1} extra={
                    <>
                        <Button type="danger" onClick={() => setShowDelete(true)} style={{marginRight: '8px'}}>삭제</Button>
                        <Button onClick={() => router.push('/board/faq/list')}>목록으로 돌아가기</Button>
                    </>
                }>
                ...
                <ButtonWrap>
                    ...
                    {modiStatus ?
                    <Button onClick={() => router.replace('/board/faq/1', undefined, {shallow: true}).then(() => window.scrollTo(0,0))}>취소</Button>
                    : <Button onClick={() => router.push('/board/faq/list')}>목록</Button>
                    }
                </ButtonWrap>
            </Detail>
