...
const PolicyDetail = () => {
    ...
    const onModi = () => {
        if (!router.query.type) router.push('/board/policy/1?type=modi');
        else {
    ...
    return (
        <Wrapper>
            <Detail>
                <Descriptions title={<Title>약관 및 정책 글 상세</Title>} bordered column={1} extra={
                    <>
                        <Button type="danger" onClick={() => setShowDelete(true)} style={{marginRight: '8px'}}>삭제</Button>
                        <Button onClick={() => router.push('/board/policy/list')}>목록으로 돌아가기</Button>
                    </>
                }>
                ...
                <ButtonWrap>
                    ...
                    {modiStatus ?
                    <Button onClick={() => router.replace('/board/policy/1', undefined, {shallow: true}).then(() => window.scrollTo(0,0))}>취소</Button>
