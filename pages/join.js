...
const Join = () => {
    ...
    const onJoin = () => {
        ...
        if (!mail) {
            return message.warning('메일주소를 입력해 주세요')
        }
        if (!phone) {
                ...
                <JoinForm>
                    <Descriptions title="루밍 파트너 회원가입" bordered column={1}>
                        ...
                        <Descriptions.Item label="메일주소">
                            <Input placeholder="메일주소를 입력해 주세요" value={mail} onChange={e => setMail(e.target.value)} />
                        </Descriptions.Item>
                        <Descriptions.Item label="담당자 연락처">
                            <Input placeholder="휴대폰 번호를 입력해 주세요" value={phone} onChange={e => setPhone(e.target.value)} />
                        </Descriptions.Item>
                    ...
                    <BtnWrap>
                        <JoinBtn type="primary" onClick={onJoin}>가입 완료</JoinBtn>
